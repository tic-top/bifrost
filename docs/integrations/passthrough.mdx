...
- `/passthrough/{provider}`
  Provider-addressed passthrough for custom or built-in providers. The provider
  segment is removed before forwarding, so the upstream receives its original
  path and body unchanged. This is useful when one Bifrost process fronts
  multiple OpenAI-compatible endpoints with different base URLs.
...
### Provider-Addressed Passthrough

- Uses the configured provider named in the URL. For example,
  `/passthrough/local-vllm/v1/chat/completions` selects `local-vllm` and sends
  `/v1/chat/completions` upstream.
- The request body is not rewritten, so the model remains the provider-native
  model ID rather than requiring a routing prefix.
