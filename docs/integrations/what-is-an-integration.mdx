...
### Muse Code catalog compatibility

Meta Muse Code checks a vendor-specific catalog before it sends its first
Responses request to a custom gateway. Bifrost serves the same configured model
list in Muse's expected shape at both `GET /muse-code/models` and
`GET /v1/muse-code/models`. Set Muse's base URL to Bifrost's standard `/v1`
base; inference continues through `POST /v1/responses`.
