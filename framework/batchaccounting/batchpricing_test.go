...
func TestBatchPricing_ClaudeHaiku45_NoBatchPricing(t *testing.T) {
	...
	// The batch really ran and consumed tokens; we just have no batch rates for the
	// model yet. The row is still written with an unknown cost so the usage is not
	// lost and the missing-cost backfill can price it once rates land — the same
	// treatment a synchronous call gets.
	...
	assert.Equal(t, "claude-haiku-4-5", logged.Model,
...
func TestBatchPricing_MultiModel(t *testing.T) {
	...
	for _, bm := range []string{"anthropic.claude-sonnet-4-6", "global.anthropic.claude-sonnet-4-6"} {
		...
		assert.Equal(t, 1.5e-06, *breakdown[bm].InputCostPerTokenBatches)
