...
import { fieldLabelByKey, PricingFieldKey } from "../../custom-pricing/overrides/pricingFields";
...
// formatPatchValue renders a patch value the way the pricing table does for
// per-token fields, and as a plain dollar amount for everything else
// (per-image, per-second, per-page, …).
function formatPatchValue(key: string, value: number): string {
	if (key.includes("_per_token") || key.includes("_per_character")) return formatTokenPriceFull(value);
	return `$${value}`;
}
