...
func (provider *OpenAIProvider) Passthrough(
	...
	url := provider.buildPassthroughURL(ctx, req)
...
func (provider *OpenAIProvider) buildPassthroughURL(ctx *schemas.BifrostContext, req *schemas.BifrostPassthroughRequest) string {
	path := req.Path
	baseURL := provider.resolveBaseURL(ctx)
	if req.UpstreamURL != "" {
